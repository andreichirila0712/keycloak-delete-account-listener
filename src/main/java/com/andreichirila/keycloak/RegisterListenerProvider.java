...
public class RegisterListenerProvider implements EventListenerProvider {
    private static final Logger logger = LoggerFactory.getLogger(RegisterListenerProvider.class);
    ...
    private void sendWebhook(Event event) {
        ...
        payload.put("eventType", EventType.REGISTER.toString());
        ...
        try {
            ...
            logger.error("Failed to serialize REGISTER payload", e);
        } catch (InterruptedException | IOException e) {
            logger.error("Failed to send webhook for user registration. UserId: {}", event.getUserId(), e);
        }
    }
}
